...
/**
 * ISO 3166-1 countries with their international calling codes, sorted by name.
 * Several dial codes are shared (e.g. +1 across the US, Canada and much of the
 * Caribbean), so `code` is the unique key — never `dial`.
 */
...
/** Country names for a select, with the primary market pinned to the top. */
export const COUNTRY_NAMES = [
  'United States',
  ...COUNTRIES.filter((c) => c.name !== 'United States').map((c) => c.name),
]

/** Unique dial codes, ordered for the phone prefix selector. +1 sorts first. */
export const DIAL_CODES = Array.from(new Set(COUNTRIES.map((c) => c.dial))).sort(
